...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    //
    cudaMalloc(&device_x, sizeof(int) * N);
    cudaMalloc(&device_y, sizeof(int) * N); 
    cudaMalloc(&device_result, sizeof(int) * N);
    cudaMemcpy(device_x, xarray, sizeof(int) * N, cudaMemcpyHostToDevice);
    cudaMemcpy(device_y, yarray, sizeof(int) * N, cudaMemcpyHostToDevice);
    ...
    //
    cudaMemcpy(resultarray, device_result, sizeof(int) * N, cudaMemcpyDeviceToHost);
